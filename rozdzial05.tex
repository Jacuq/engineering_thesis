...
W ramach implementacji backendu stworzono testy jednostkowe sprawdzaj¹ce poprawnoœæ implementacji klasy \texttt{notes\_calc}. Do implementacji wykorzystano bibliotekê \texttt{pytest} -- jedn¹ z~najpopularniejszych bibliotek do testowania oprogramowania. 

Testy zaprojektowano w taki sposób, aby zapewniæ odpowiednie dzia³anie funkcji obliczaj¹cych skale, akordy oraz interwa³y, w~ka¿dym z~czterech mo¿liwych przypadków: oba wprowadzone argumenty s¹ poprawne, tylko pierwszy argument jest poprawny, tylko drugi argument jest poprawny oraz oba argumenty s¹ niepoprawne. Testy zaimplementowano dla wszystkich funkcji licz¹cych: \texttt{find\_chord}, \texttt{find\_scale} oraz \texttt{find\_interval}. 
...
\begin{figure}[ht]
			\centering
			\includegraphics[width=.9\textwidth]{rys05/UTs.png}
